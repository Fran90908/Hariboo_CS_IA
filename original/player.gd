...
func _ready():
	...
	ShopMenu.tutorial_finished.connect(Callable(self, "hide_tutorial_label"))
...
func _physics_process(delta):
	...
	if is_dashing:
		# Calculate how long dash was charged (0–1 ratio)
		var charge_ratio = clamp(dash_charge_timer / dash_charge_max, 0.0, 1.0)
		# Non-linear dash speed based on charge
		...
		velocity = direction * (600 if global.speed else 400)
	# Move character using physics engine
	move_and_slide()
